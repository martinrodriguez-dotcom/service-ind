...
const App = () => {
    ...
    const ModalComp = window.ModalComp;
    const FuelTankCapsule = window.FuelTankCapsule;
    ...
    const [bndSettings, setBndSettings] = useState({
        ...
        description: "Mantenimiento Industrial & Gestión de Activos",
    ...
    useEffect(() => {
        const initData = () => {
            ...
            window.fb.onAuthStateChanged(window.auth, (u) => {
                setUser(u);
                if (u) {
                    const configRef = window.fb.doc(window.db, "artifacts", window.APP_ID, "public", "data", "config", "bnd_info");
                    window.fb.getDoc(configRef).then(snap => { if(snap.exists()) setBndSettings(snap.data()); });

                    const q = window.fb.collection(window.db, "artifacts", window.APP_ID, "public", "data", "companies");
        ...
        if (window.db) initData();
        else window.addEventListener('firebase-ready', initData);
    }, []);
            ...
            (emp.vehiculos || []).forEach(veh => {
                if (!veh.operativo) {
                    list.push({ 
                        ...
                        breakageMotive: (veh.eventos || []).slice().reverse().find(e => e.tipo === 'BAJA')?.motivo || "Falla no detallada"
                    });
                }
    ...
    useEffect(() => {
        if (modalType === 'details' && activeCompany && chartRef.current) {
            ...
            if (window.myChartCompV2) window.myChartCompV2.destroy();
            window.myChartCompV2 = new window.Chart(ctx, {
                type: 'line',
                data: {
    ...
    const saveBndSettings = async () => {
        const docRef = window.fb.doc(window.db, "artifacts", window.APP_ID, "public", "data", "config", "bnd_info");
        await window.fb.setDoc(docRef, bndSettings);
        setActiveTab('dashboard');
    };

    const handleAddCompany = async () => {
        if(!form.nombre) return;
        const coll = window.fb.collection(window.db, "artifacts", window.APP_ID, "public", "data", "companies");
        await window.fb.addDoc(coll, { 
    ...
    const handleDailyLog = async () => {
        ...
        await window.fb.updateDoc(window.fb.doc(window.db, "artifacts", window.APP_ID, "public", "data", "companies", activeCompany.id), { vehiculos: updated, currentFuel: newFuel });
    ...
    const handleToggleStatus = async (vId, currentStatus) => {
        if(currentStatus) { setActiveVehicleId(vId); setModalType('downtime'); }
            ...
            await window.fb.updateDoc(window.fb.doc(window.db, "artifacts", window.APP_ID, "public", "data", "companies", activeCompany.id), { vehiculos: updated });
        }
    };
    ...
    const handleUpdateWorkflow = async (cId, vId, newStatus) => {
        ...
        await window.fb.updateDoc(window.fb.doc(window.db, "artifacts", window.APP_ID, "public", "data", "companies", cId), { vehiculos: updated });
    };
    ...
    const handleRepairSubmit = async () => {
        ...
        await window.fb.updateDoc(window.fb.doc(window.db, "artifacts", window.APP_ID, "public", "data", "companies", selectedCompanyId), { vehiculos: updated });
        setModalType(null); setForm({...form, nota: ''});
        setActiveTab('companies');
    };

    const handleAddVehicle = async () => {
        const ref = window.fb.doc(window.db, "artifacts", window.APP_ID, "public", "data", "companies", activeCompany.id);
        ...
        await window.fb.updateDoc(ref, { vehiculos: window.fb.arrayUnion(v) });
        setModalType(null);
    };
    ...
    const refillTank = async () => {
        await window.fb.updateDoc(window.fb.doc(window.db, "artifacts", window.APP_ID, "public", "data", "companies", activeCompany.id), { currentFuel: activeCompany.tankCapacity });
        setModalType(null);
    };
        ...
        <div className="flex h-screen bg-slate-50 relative overflow-hidden leading-none text-slate-900">
            ...
            <aside className={`sidebar-mobile lg:w-64 bg-white border-r border-slate-200 p-6 flex flex-col lg:static ${sidebarOpen ? 'open' : ''}`}>
                ...
                <div className="pt-6 border-t leading-none">
                     <button onClick={startScanner} className="w-full p-3.5 bg-yellow-400 text-black rounded-xl flex items-center justify-center gap-2 font-black text-[9px] uppercase shadow-md shadow-yellow-100 transition-transform active:scale-95 leading-none transition-all"><Icon name="qr" size={14}/> <span>Escanear</span></button>
                </div>
            </aside>
            ...
            <main className="flex-1 p-5 sm:p-8 lg:p-10 overflow-y-auto relative leading-none">
                                    ...
                                    <div key={i} className={`glass-card p-5 flex flex-col md:flex-row justify-between items-center gap-6 border-l-[10px] ${colorClass}`}>
                                        ...
                                        <div className="flex items-center gap-6 leading-none">
                                            <div className="text-right leading-none shrink-0 min-w-[100px] leading-none">
                                                {a.type === 'BREAKDOWN' ? (
                                                    <div className="space-y-1.5 leading-none"><p className="text-[10px] font-black text-red-600 uppercase tracking-widest leading-none">{a.workflowStatus}</p><p className="text-[9px] font-bold text-slate-400 italic max-w-[180px] truncate leading-none">{a.breakageMotive}</p></div>
                                                ) : a.type === 'MAINTENANCE' ? (
                                            ...
                                            {a.type === 'BREAKDOWN' ? (
                                                ...
                                                <button onClick={() => { setSelectedCompanyId(a.companyId); setActiveTab('companies'); }} className="btn-premium px-5 py-2.5 text-[9px] uppercase leading-none shadow-md">Detalle</button>
                                            )}
                                        </div>
                                    </div>
                                );
                ...
                {activeTab === 'companies' && !activeCompany && (
                    <div className="max-w-6xl mx-auto space-y-10 animate-fade-in pb-20 pt-10 lg:pt-0 leading-none">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 leading-none">
                            {companies.map(emp => (
                                <div key={emp.id} className="glass-card p-6 flex flex-col min-h-[300px] leading-none transition-all">
                                    <div className="flex justify-between items-start mb-6 leading-none">
                                        <div className="w-12 h-12 bg-slate-900 rounded-xl flex items-center justify-center text-yellow-400 shadow-xl leading-none"><Icon name="company" size={24}/></div>
                                        <FuelTankCapsule capacity={emp.tankCapacity || 1000} current={emp.currentFuel || 0} />
                                    </div>
                                    <h3 className="text-xl font-black uppercase italic mb-1.5 truncate tracking-tight leading-none leading-none">{emp.nombre}</h3>
                                    <p className="text-[9px] font-black text-slate-300 uppercase tracking-widest mb-8 leading-none leading-none">CUIT: {emp.cuit}</p>
                                    <div className="mt-auto flex gap-3 leading-none leading-none">
                                        <button onClick={() => setSelectedCompanyId(emp.id)} className="flex-1 btn-premium py-3.5 text-[9px] uppercase tracking-widest leading-none shadow-md">Flota</button>
                                        <button onClick={() => { setSelectedCompanyId(emp.id); setModalType('details'); }} className="p-3 bg-slate-50 border border-slate-100 rounded-xl hover:bg-yellow-400 transition-all leading-none"><Icon name="chart" size={18}/></button>
                                    </div>
                                </div>
                            ))}
                        </div>
                ...
                {activeTab === 'config' && (
                    <div className="max-w-3xl mx-auto space-y-10 animate-fade-in pt-10 lg:pt-0 leading-none leading-none">
                        <h2 className="text-4xl font-black uppercase italic tracking-tighter leading-none leading-none">Configuración</h2>
                        <div className="glass-card p-10 space-y-8 leading-none shadow-2xl leading-none">
                            <div className="grid grid-cols-1 gap-6 leading-none">
                                <div className="space-y-2 leading-none leading-none leading-none leading-none"><label className="text-[10px] font-black uppercase text-slate-400 italic leading-none leading-none ml-2">Razón Social</label><input className="w-full input-premium font-black shadow-inner leading-none leading-none" value={bndSettings.name} onChange={e => setBndSettings({...bndSettings, name: e.target.value})} /></div>
                                <div className="space-y-2 leading-none leading-none leading-none leading-none"><label className="text-[10px] font-black uppercase text-slate-400 italic leading-none leading-none ml-2">Eslogan Corporativo</label><input className="w-full input-premium leading-none shadow-inner leading-none" value={bndSettings.description} onChange={e => setBndSettings({...bndSettings, description: e.target.value})} /></div>
                                <div className="space-y-2 leading-none leading-none leading-none leading-none"><label className="text-[10px] font-black uppercase text-slate-400 italic leading-none leading-none ml-2">Dirección Administrativa</label><input className="w-full input-premium leading-none shadow-inner leading-none" value={bndSettings.address} onChange={e => setBndSettings({...bndSettings, address: e.target.value})} /></div>
                                <div className="grid grid-cols-2 gap-4 leading-none leading-none">
                                    <div className="space-y-2 leading-none leading-none leading-none leading-none"><label className="text-[10px] font-black uppercase text-slate-400 italic leading-none leading-none ml-2 leading-none">Teléfono</label><input className="w-full input-premium leading-none leading-none shadow-inner leading-none" value={bndSettings.phone} onChange={e => setBndSettings({...bndSettings, phone: e.target.value})} /></div>
                                    <div className="space-y-2 leading-none leading-none leading-none leading-none"><label className="text-[10px] font-black uppercase text-slate-400 italic leading-none leading-none ml-2 leading-none">Email Técnico</label><input className="w-full input-premium leading-none leading-none shadow-inner leading-none" value={bndSettings.email} onChange={e => setBndSettings({...bndSettings, email: e.target.value})} /></div>
                                </div>
                            </div>
                            <button onClick={saveBndSettings} className="btn-premium w-full p-6 text-[11px] uppercase tracking-widest mt-4 leading-none shadow-2xl transition-all">Impactar Datos de Membrete</button>
                        </div>
                    </div>
                )}

                {activeCompany && (
                    <div className="max-w-6xl mx-auto space-y-8 animate-in slide-in-from-right-8 duration-300 pb-20 pt-10 lg:pt-0 leading-none leading-none">
                        <button onClick={() => setSelectedCompanyId(null)} className="flex items-center gap-2 font-black text-[9px] uppercase hover:text-yellow-600 transition-all mb-4 group leading-none leading-none leading-none"><Icon name="chevronLeft" size={14}/> Directorio</button>
                        <div className="glass-card p-8 flex flex-col xl:flex-row justify-between items-start xl:items-end gap-10 bg-slate-900 text-white relative overflow-hidden border-none leading-none shadow-xl">
                            <div className="absolute top-0 right-0 w-[300px] h-[300px] bg-yellow-400/5 blur-[80px] pointer-events-none leading-none"></div>
                            <div className="space-y-4 relative z-10 leading-none"><h2 className="text-4xl sm:text-5xl font-black uppercase italic leading-none tracking-tighter leading-none leading-none leading-none">{activeCompany.nombre}</h2><div className="flex flex-wrap gap-2 leading-none leading-none"><div className="bg-white/5 px-3 py-1.5 rounded-lg border border-white/10 text-[7px] font-black uppercase tracking-widest leading-none">{activeCompany.cuit}</div><div className="bg-yellow-400/10 text-yellow-400 px-3 py-1.5 rounded-lg border border-yellow-400/10 text-[7px] font-black uppercase tracking-widest italic leading-none">{activeCompany.responsable}</div></div></div>
                            <button onClick={() => setModalType('vehicle')} className="btn-accent px-8 py-4 text-[9px] uppercase relative z-10 leading-none shadow-2xl leading-none">Vincular Equipo</button>
                        </div>
                        ...
                        <div className="grid gap-6 leading-none">
                            {(activeCompany.vehiculos || []).map(v => {
                                ...
                                return (
                                    <div key={v.id} className="glass-card p-6 sm:p-8 relative overflow-hidden border border-slate-100 leading-none transition-all">
                                        <div className={`absolute top-0 right-0 px-5 py-1.5 text-[7px] font-black uppercase italic tracking-widest rounded-bl-xl leading-none leading-none ${v.operativo ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-600 animate-pulse'}`}>{v.operativo ? '● ACTIVO' : '● ROTURA'}</div>
                                        <div className="flex flex-col xl:flex-row gap-8 leading-none leading-none">
                                            <div className="flex-1 space-y-6 leading-none leading-none">
                                                <div className="flex items-center gap-5 leading-none leading-none"><div className="w-12 h-12 bg-slate-50 rounded-xl flex items-center justify-center text-slate-800 shadow-inner border border-slate-100 leading-none leading-none"><Icon name="truck" size={20}/></div><div className="truncate w-full leading-none leading-none"><h4 className="text-xl font-black italic tracking-tighter mb-1.5 uppercase leading-none leading-none leading-none leading-none">{v.nombre}</h4><div className="flex items-center gap-4 leading-none leading-none"><span className="mono font-black text-lg leading-none leading-none leading-none leading-none">{(v.horometroTotal || 0).toLocaleString()} <span className="text-[9px] text-slate-300 font-bold ml-0.5 leading-none">HS</span></span><div className="flex gap-2 border-l pl-4 border-slate-100 leading-none leading-none"><button onClick={() => downloadPDF(activeCompany, v)} className="p-1.5 bg-slate-50 rounded-lg hover:bg-slate-900 hover:text-white transition-all leading-none leading-none leading-none shadow-sm leading-none"><Icon name="download" size={14}/></button><button onClick={() => { setActiveVehicleId(v.id); setModalType('qr'); setTimeout(() => generateQR(v.id), 100); }} className="p-1.5 bg-slate-50 rounded-lg hover:bg-yellow-400 transition-all leading-none leading-none leading-none shadow-sm leading-none"><Icon name="qr" size={14}/></button></div></div></div></div>
                                                <div className="space-y-2.5 leading-none leading-none"><div className="flex justify-between items-end leading-none leading-none leading-none"><p className="text-[8px] font-black uppercase italic text-slate-400 leading-none tracking-widest leading-none leading-none leading-none leading-none">Estado Ciclo ({v.serviceInterval}h)</p><p className={`text-md font-black mono leading-none leading-none leading-none leading-none leading-none ${perc < 25 ? 'text-red-600' : 'text-slate-900'}`}>{perc.toFixed(0)}%</p></div><div className="h-1.5 w-full bg-slate-50 rounded-full overflow-hidden leading-none leading-none shadow-inner"><div className={`h-full transition-all duration-1000 ${perc < 25 ? 'bg-red-500' : 'bg-slate-900'}`} style={{width: `${perc}%`}}></div></div></div>
                                            </div>
                                            <div className="grid grid-cols-2 sm:grid-cols-4 xl:grid-cols-1 xl:w-48 gap-2.5 shrink-0 leading-none text-center">
                                                <button onClick={() => { setActiveVehicleId(v.id); setModalType('log'); }} className="p-2.5 bg-slate-900 text-white rounded-lg font-black text-[7px] uppercase hover:bg-yellow-400 hover:text-black leading-none transition-all leading-none leading-none leading-none">Cargar</button>
                                                <button onClick={() => { setActiveVehicleId(v.id); setForm({...form, horas: v.horometroTotal}); setModalType('historical'); }} className="p-2.5 bg-white border border-slate-200 rounded-lg font-black text-[7px] uppercase hover:bg-slate-50 leading-none transition-all leading-none leading-none shadow-sm">Historial</button>
                                                <button onClick={() => handleToggleStatus(v.id, v.operativo)} className={`p-2.5 rounded-lg font-black text-[7px] uppercase transition-all leading-none leading-none leading-none shadow-sm leading-none ${v.operativo ? 'bg-red-50 text-red-600 border border-red-100' : 'bg-emerald-500 text-white'}`}>{v.operativo ? 'Rotura' : 'Revivir'}</button>
                                                <button onClick={() => { setActiveVehicleId(v.id); setModalType('service'); }} className="p-2.5 bg-yellow-400 text-black rounded-lg font-black text-[7px] uppercase shadow-lg shadow-yellow-100 transition-all leading-none leading-none leading-none">Service</button>
                                            </div>
                                        </div>
                                        <button onClick={() => toggleHistory(v.id)} className="flex items-center gap-2 text-[8px] font-black uppercase text-slate-300 hover:text-slate-900 transition-all border-t mt-4 pt-3 tracking-widest leading-none leading-none leading-none leading-none leading-none leading-none"><Icon name="history" size={12}/> Auditoría <Icon name="chevronDown" size={12} className={isExp ? 'rotate-180 transition-all' : 'transition-all'}/></button>
                                        {isExp && (
                                            <div className="mt-4 overflow-x-auto rounded-xl border border-slate-100 bg-slate-50/20 p-2 shadow-inner animate-fade-in leading-none leading-none">
                                                <table className="w-full history-table leading-none leading-none leading-none">
                                                    <thead><tr><th>Fecha</th><th>Tipo</th><th>Lectura</th><th>Detalles</th></tr></thead>
                                                    <tbody>{[...(v.eventos || [])].reverse().map((ev, idx) => ( <tr key={idx} className="hover:bg-white transition-all leading-none rounded-xl leading-none leading-none leading-none leading-none"><td className="font-bold opacity-30 italic leading-none leading-none leading-none">{ev.fecha}</td><td className="leading-none leading-none leading-none"><span className="font-black text-[6px] uppercase border px-1.5 py-0.5 rounded bg-white leading-none shadow-sm leading-none leading-none">{ev.tipo}</span></td><td className="mono font-black text-slate-900 leading-none leading-none leading-none leading-none">{ev.horas?.toLocaleString() || 0}</td><td className="text-slate-500 italic text-[9px] leading-tight leading-none leading-none leading-none leading-none leading-none leading-none">{ev.litros ? `-${ev.litros}L | ` : ''}{ev.insumos?.length ? ` Reps: ${ev.insumos.join(", ")} | ` : ''}{ev.nota || ev.motivo || '-'}</td></tr> ))}</tbody>
                                                </table>
                                            </div>
                                        )}
                                    </div>
                                ...
                                <div key={i} className="glass-card p-6 flex flex-col gap-4 leading-none leading-none shadow-xl shadow-slate-100">
                                    <div className="flex justify-between items-start gap-4 leading-none">
                                        ...
                                        <div className="text-right shrink-0 leading-none leading-none">
                                            <p className="text-[8px] font-black uppercase tracking-widest mb-1.5 text-slate-400 leading-none italic">Est. Service</p>
                                    ...
                                    <div className="p-2.5 bg-slate-50 rounded-xl border flex justify-between items-center text-[8px] font-black uppercase tracking-widest italic leading-none leading-none leading-none">
                                        <div>Promedio Día: <span className="text-blue-600 block text-[10px] mt-1.5 leading-none leading-none">{p.avgUsage.toFixed(1)} HS</span></div>
                                        <div className="text-right leading-none leading-none">Resta Ciclo: <span className="text-blue-600 block text-[10px] mt-1.5 leading-none leading-none">{Math.max(0, p.restHs).toFixed(1)} HS</span></div>
                                    </div>
                                </div>
                            ))}
                        </div>
                ...
                <ModalComp title={
                    ...
                    {modalType === 'log' && (
                        <div className="space-y-8 text-center animate-fade-in leading-none leading-none">
                            <div className="grid grid-cols-2 gap-6 leading-none leading-none">
                                <div className="space-y-3 leading-none leading-none"><label className="text-[10px] font-black text-slate-400 uppercase leading-none leading-none">Lectura Horas</label><div className="relative leading-none leading-none"><div className="absolute top-2 right-6 text-[9px] font-black text-slate-300 italic uppercase leading-none">Ant: {activeCompany?.vehiculos?.find(v => v.id === activeVehicleId)?.horometroTotal || 0}</div><input className="w-full p-6 input-premium mono text-4xl text-center shadow-inner border-2 border-slate-100 leading-none leading-none" type="number" value={form.horas} onChange={e => setForm({...form, horas: e.target.value})} /></div></div>
                                <div className="space-y-3 leading-none leading-none"><label className="text-[10px] font-black text-slate-400 uppercase leading-none leading-none">Carga Diésel (L)</label><input className="w-full p-6 input-premium mono text-4xl text-center shadow-inner border-2 border-slate-100 leading-none leading-none" type="number" value={form.litros} onChange={e => setForm({...form, litros: e.target.value})} /></div>
                            </div>
                            <textarea className="w-full input-premium min-h-[100px] font-bold text-xs uppercase shadow-inner leading-none leading-none" placeholder="Notas jornada..." value={form.nota} onChange={e => setForm({...form, nota: e.target.value})} />
                            <button disabled={!form.horas || !form.litros} onClick={handleDailyLog} className="btn-accent w-full p-5 text-[10px] uppercase shadow-lg mt-4 leading-none leading-none">Impactar Registro</button>
                        </div>
                    )}
                    ...
                    {modalType === 'repair_finish' && (
                        <div className="space-y-10 animate-fade-in leading-none leading-none">
                            <div className="p-8 bg-emerald-50 rounded-2xl border border-emerald-200 flex items-center gap-6 shadow-xl leading-none shadow-emerald-50 leading-none"><Icon name="check" size={50} className="text-emerald-600"/><div className="leading-none leading-none leading-none"><p className="text-2xl font-black text-emerald-600 uppercase italic leading-none leading-none">Equipo Reparado</p><p className="text-[10px] font-bold text-emerald-500 uppercase mt-2 leading-none leading-none">Unidad habilitada hoy.</p></div></div>
                            <textarea className="w-full input-premium min-h-[180px] font-bold uppercase text-[10px] shadow-inner leading-none leading-none" placeholder="Informe técnico de reparación..." value={form.nota} onChange={e => setForm({...form, nota: e.target.value})} />
                    ...
                    {modalType === 'qr' && (
                        <div className="space-y-12 text-center flex flex-col items-center animate-fade-in py-8 leading-none leading-none">
                            <div className="p-10 bg-white rounded-[3rem] shadow-2xl border border-slate-100 leading-none shadow-slate-100" id={`qr-container-${activeVehicleId}`}></div>
                            <div className="space-y-3 leading-none leading-none"><p className="text-[11px] font-bold text-slate-400 max-w-[280px] leading-relaxed">Etiqueta ID BND-{activeVehicleId?.slice(-4)}</p></div>
                            <button onClick={() => window.print()} className="btn-premium px-16 py-6 text-xs uppercase leading-none shadow-2xl transition-all">Imprimir Etiqueta</button>
                        </div>
                    )}
