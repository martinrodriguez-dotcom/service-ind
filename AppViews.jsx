    ...
    <div className="grid grid-cols-1 gap-5">
            ...
            {a.type === 'BREAKDOWN' ? (
              <div className="flex flex-col sm:flex-row bg-slate-100/50 p-1.5 rounded-xl w-full gap-2 border border-slate-200/50">
                {['PENDIENTE', 'REVISION', 'REPARADO'].map(s => (
                  <button key={s} onClick={() => handleUpdateWorkflow(a.companyId, a.id, s)} className={`px-4 py-2.5 rounded-[0.85rem] text-[10px] font-black uppercase w-full transition-all ${a.workflowStatus === s ? 'bg-red-500 text-white shadow-lg shadow-red-500/30 scale-[1.02]' : 'bg-transparent text-slate-500 hover:bg-slate-200'}`}>{s}</button>
                ))}
              </div>
            ) : (
              <button onClick={() => { setSelectedCompanyId(a.companyId); setActiveVehicleId(null); setActiveTab('companies'); }} className="btn-premium px-6 py-3.5 rounded-xl w-full md:w-auto text-xs font-black tracking-widest">VER FLOTA</button>
            )}
          </div>
        </div>
      ))}
      {alerts.length === 0 && (
        <div className="text-center p-12 bg-white/40 backdrop-blur-md rounded-[2rem] border border-emerald-100 shadow-xl shadow-emerald-900/5">
          <div className="w-16 h-16 bg-emerald-100 text-emerald-500 rounded-2xl mx-auto flex items-center justify-center mb-4"><Icon name="check" size={32}/></div>
...
const CompaniesView = ({ companies, role, setForm, setModalType, setSelectedCompanyId, setActiveVehicleId, handleDeleteCompany }) => (
  <div className="max-w-6xl mx-auto animate-fade-in">
    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8">
      ...
      {role === 'admin' && (
        <button onClick={() => { setForm(prev => ({...prev, nombre:'', cuit:'', responsable:'', tankCapacity:1000, criticalFuelPerc: 25})); setModalType('company'); }} className="btn-accent px-6 py-3.5 rounded-xl w-full sm:w-auto text-xs font-black tracking-widest">+ NUEVO CLIENTE</button>
      )}
    </div>
      ...
      {companies.map(emp => (
        <div key={emp.id} className="glass-card p-6 md:p-8 relative group flex flex-col h-full">
          <div className="flex justify-between items-start mb-8">
            <div className="w-14 h-14 bg-slate-900 rounded-[1.25rem] flex items-center justify-center text-yellow-400 shadow-xl shadow-slate-900/20"><Icon name="company" size={26}/></div>
          ...
          <div className="flex gap-3 w-full mt-auto">
            <button onClick={() => { setSelectedCompanyId(emp.id); setActiveVehicleId(null); }} className="btn-premium flex-1 px-5 py-3.5 rounded-xl text-sm font-black uppercase tracking-wider">Flota</button>
            {role === 'admin' && (
              <>
                <button onClick={() => { setSelectedCompanyId(emp.id); setForm(prev => ({...prev, nombre: emp.nombre, cuit: emp.cuit, responsable: emp.responsable, tankCapacity: emp.tankCapacity, criticalFuelPerc: emp.criticalFuelPerc || 25})); setModalType('edit_company'); }} className="px-4 py-3.5 bg-slate-100/50 hover:bg-yellow-400 rounded-xl transition-colors flex items-center justify-center border border-slate-200/50"><Icon name="settings" size={18}/></button>
                <button onClick={() => handleDeleteCompany(emp.id)} className="px-4 py-3.5 bg-red-50 hover:bg-red-500 text-red-500 hover:text-white rounded-xl transition-colors flex items-center justify-center border border-red-100"><Icon name="x" size={18}/></button>
              </>
            )}
          </div>
        </div>
...
const CompanyDetailView = ({ role, activeCompany, activeVehicle, activeVehicleId, isolatedVehicleId, displayedVehicles, expandedVehicles, setSelectedCompanyId, setIsolatedVehicleId, setActiveVehicleId, setForm, setModalType, handleDeleteVehicle, handleToggleStatus, downloadPDF, generateQR, toggleHistory }) => {
  
  if (!activeVehicleId) {
    return (
      <div className="max-w-6xl mx-auto animate-fade-in">
        {role === 'admin' && <button onClick={() => setSelectedCompanyId(null)} className="flex items-center gap-2 text-xs md:text-sm font-bold uppercase mb-8 text-slate-400 hover:text-slate-900 transition-colors tracking-widest"><Icon name="chevronLeft" size={16}/> Volver al Directorio</button>}
        ...
        <div className="glass-card p-8 md:p-10 bg-gradient-to-br from-slate-900 to-slate-800 text-white mb-8 relative overflow-hidden flex flex-col md:flex-row justify-between items-start md:items-center gap-6 shadow-2xl shadow-slate-900/20 border-none">
          ...
          {role === 'admin' && <button onClick={() => { setForm(prev => ({...prev, nombre:'', marca:'', modelo:'', serie:'', patente:'', serviceInterval: 250, horometro: ''})); setModalType('vehicle'); }} className="btn-accent px-6 py-4 rounded-xl text-xs font-black tracking-widest z-10 w-full md:w-auto shadow-xl shadow-yellow-400/20">+ VINCULAR EQUIPO</button>}
        </div>
            ...
            return (
              <div key={v.id} onClick={() => setActiveVehicleId(v.id)} className="glass-card p-6 cursor-pointer hover:border-yellow-400 transition-all group flex flex-col h-full">
                ...
                <div className="space-y-2 mt-auto bg-slate-50 p-4 rounded-2xl border border-slate-100/50">
                    <div className="flex justify-between items-end"><span className="text-[9px] font-bold uppercase text-slate-400 tracking-widest">Vida Útil ({v.serviceInterval}h)</span><span className={`text-[11px] font-black mono ${perc < 20 ? 'text-red-500' : 'text-slate-800'}`}>{perc.toFixed(0)}%</span></div>
                    <div className="h-2 bg-slate-200/50 rounded-full overflow-hidden shadow-inner"><div className={`h-full transition-all duration-1000 ${perc < 20 ? 'bg-red-500 shadow-[0_0_10px_rgba(239,68,68,0.5)]' : 'bg-slate-800'}`} style={{width: `${perc}%`}} /></div>
                </div>
              </div>
            );
          })}
      ...
      <div className="glass-card p-8 md:p-12 relative overflow-hidden shadow-2xl">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-8 mb-10 border-b border-slate-100 pb-10 mt-4">
          ...
          <div className="bg-slate-50 p-5 rounded-[1.5rem] text-center w-full md:w-auto border border-slate-200/60 shadow-inner">
              <p className="text-[10px] font-black uppercase text-slate-400 tracking-widest mb-1">Horómetro Actual</p>
              <p className="font-black text-slate-800 text-4xl mono">{(activeVehicle.horometroTotal || 0).toLocaleString()} <span className="text-sm text-slate-400 uppercase tracking-widest">HS</span></p>
          </div>
        </div>

        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-10">
          <button onClick={() => setModalType('log')} className="btn-premium px-4 py-5 rounded-2xl text-xs font-black tracking-widest w-full">CARGAR DATOS</button>
          <button onClick={() => setModalType('service')} className="btn-accent px-4 py-5 rounded-2xl text-xs font-black tracking-widest w-full">SERVICE</button>
          <button onClick={() => handleToggleStatus(activeVehicle.id, activeVehicle.operativo)} className={`px-4 py-5 rounded-2xl font-black text-xs tracking-widest uppercase transition-all w-full shadow-md ${activeVehicle.operativo ? 'bg-red-50 text-red-600 border border-red-100 hover:bg-red-600 hover:text-white' : 'bg-emerald-500 text-white shadow-emerald-500/30'}`}>
              {activeVehicle.operativo ? 'EVENTO' : 'REVIVIR'}
          </button>
          <button onClick={() => { setForm(prev => ({...prev, horas: activeVehicle.horometroTotal, fecha: new Date().toISOString().split('T')[0]})); setModalType('historical'); }} className="bg-slate-100 text-slate-600 hover:bg-slate-200 hover:text-slate-900 px-4 py-5 rounded-2xl text-xs font-black tracking-widest w-full transition-colors border border-slate-200/50">HISTORIAL</button>
        </div>

        <div className="flex flex-wrap gap-4 mb-10 bg-slate-50/50 p-5 rounded-[1.5rem] border border-slate-100">
          <button onClick={() => downloadPDF(activeCompany, activeVehicle)} className="flex-1 px-5 py-4 bg-white hover:bg-slate-900 hover:text-white rounded-xl transition-all flex items-center justify-center border border-slate-200 shadow-sm text-[10px] font-black uppercase tracking-widest gap-3"><Icon name="download" size={16}/> Expediente PDF</button>
          <button onClick={() => { setModalType('qr'); setTimeout(() => generateQR(activeVehicle.id), 100); }} className="flex-1 px-5 py-4 bg-white hover:bg-yellow-400 rounded-xl transition-all flex items-center justify-center border border-slate-200 shadow-sm text-[10px] font-black uppercase tracking-widest gap-3"><Icon name="qr" size={16}/> Etiqueta QR</button>
          
          {role === 'admin' && (
            <div className="flex flex-1 gap-3">
              <button onClick={() => { setForm(prev => ({...prev, nombre: activeVehicle.nombre, marca: activeVehicle.marca, modelo: activeVehicle.modelo, serie: activeVehicle.serie, patente: activeVehicle.patente, serviceInterval: activeVehicle.serviceInterval})); setModalType('edit_vehicle'); }} className="flex-1 px-5 py-4 bg-white hover:bg-yellow-400 rounded-xl transition-colors flex items-center justify-center shadow-sm border border-slate-200 text-slate-500 hover:text-black gap-2 text-[10px] font-black uppercase tracking-widest"><Icon name="settings" size={16}/> Editar</button>
              <button onClick={() => handleDeleteVehicle(activeVehicle.id)} className="flex-1 px-5 py-4 bg-red-50 hover:bg-red-500 text-red-500 hover:text-white rounded-xl transition-colors flex items-center justify-center shadow-sm border border-red-100 gap-2 text-[10px] font-black uppercase tracking-widest"><Icon name="x" size={16}/> Eliminar</button>
            </div>
          )}
        </div>
        ...
        <div className="space-y-6">
          ...
          <div className="overflow-x-auto rounded-2xl border border-slate-200/60 bg-white shadow-sm">
            <table className="w-full history-table min-w-[500px]">
              <thead className="bg-slate-50"><tr><th>Fecha</th><th>Tipo de Evento</th><th>Lectura</th><th>Nota del Operario</th></tr></thead>
              <tbody>
                {([...(activeVehicle.eventos || [])].reverse().map((ev, idx) => ( 
                  <tr key={idx} className="hover:bg-slate-50/50 transition-colors">
                      <td className="font-bold text-slate-400 text-xs">{ev.fecha.slice(0,5)}</td>
                      <td><span className="font-black text-[9px] uppercase tracking-widest border border-slate-200 px-2.5 py-1.5 rounded-lg bg-white text-slate-600 shadow-sm">{ev.tipo.slice(0,4)}</span></td>
                      <td className="mono font-black text-sm text-slate-800">{ev.horas}</td>
                      <td className="text-slate-500 font-medium text-xs leading-relaxed max-w-[250px] truncate">
                          {ev.litros ? <span className="font-bold text-red-500 mr-2">-{ev.litros}L</span> : ''}
                          {ev.nota || ev.motivo || '-'}
                      </td>
                  </tr> 
                )))}
    ...
    <div className="space-y-6">
      ...
      <div className="glass-card p-8 md:p-10 space-y-8">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-2"><label className="text-[10px] font-bold uppercase tracking-widest text-slate-400 ml-2">Razón Social</label><input className="input-premium px-5 py-4 w-full font-black text-lg text-slate-800" value={bndSettings.name} onChange={e => setBndSettings({...bndSettings, name: e.target.value})} /></div>
          <div className="space-y-2"><label className="text-[10px] font-bold uppercase tracking-widest text-slate-400 ml-2">Descripción</label><input className="input-premium px-5 py-4 w-full text-sm font-medium text-slate-600" value={bndSettings.description} onChange={e => setBndSettings({...bndSettings, description: e.target.value})} /></div>
          <div className="space-y-2"><label className="text-[10px] font-bold uppercase tracking-widest text-slate-400 ml-2">Dirección Física</label><input className="input-premium px-5 py-4 w-full text-sm font-medium text-slate-600" value={bndSettings.address} onChange={e => setBndSettings({...bndSettings, address: e.target.value})} /></div>
          <div className="space-y-2"><label className="text-[10px] font-bold uppercase tracking-widest text-slate-400 ml-2">Teléfono de Contacto</label><input className="input-premium px-5 py-4 w-full text-sm font-medium text-slate-600" value={bndSettings.phone} onChange={e => setBndSettings({...bndSettings, phone: e.target.value})} /></div>
        </div>
    ...
    <div className="space-y-6">
      ...
      <div className="glass-card overflow-x-auto shadow-sm">
        <table className="w-full history-table min-w-[600px]">
          <thead className="bg-slate-50"><tr><th>Nombre y Apellido</th><th>Email</th><th>Rol / Flota Asignada</th><th>Acciones</th></tr></thead>
          <tbody>
            {usersList.map(u => (
              <tr key={u.uid} className="hover:bg-slate-50/50 transition-colors">
                ...
                <td>
                    <div className="flex flex-col gap-1.5 items-start">
                        <span className={`text-[9px] font-black uppercase tracking-widest px-2.5 py-1 rounded-md border ${u.role === 'admin' ? 'border-yellow-400 bg-yellow-50 text-yellow-700' : u.role === 'suspendido' ? 'border-red-500 bg-red-50 text-red-600' : 'bg-slate-100 text-slate-600 border-slate-200'}`}>{u.role}</span>
                        {u.role === 'operario' && (<span className="text-[10px] font-bold text-slate-400 truncate max-w-[200px]">{companies.find(c => c.id === u.companyId)?.nombre || 'Sin Empresa Asignada'}</span>)}
                    </div>
                </td>
                <td><button onClick={() => { setEditingUserId(u.uid); setNewUser({ email: u.email, nombre: u.nombre || '', role: u.role, companyId: u.companyId || '', password: '' }); setModalType('edit_user'); }} className="text-[10px] font-black uppercase tracking-widest bg-white border border-slate-200 hover:border-yellow-400 hover:text-yellow-600 text-slate-600 px-4 py-2 rounded-lg transition-colors shadow-sm">EDITAR PERFIL</button></td>
              </tr>
            ))}
          </tbody>
        </table>
...
const CalendarView = ({ role, userCompanyId, companies, selectedCompanyId, setSelectedCompanyId, companyProjections }) => {
      ...
      <div className="max-w-6xl mx-auto animate-fade-in">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {(role === 'admin' ? companies : companies.filter(c => c.id === userCompanyId)).map(emp => (
            <div key={emp.id} className="glass-card p-6 md:p-8 relative group cursor-pointer hover:border-blue-400 transition-all hover:-translate-y-1" onClick={() => setSelectedCompanyId(emp.id)}>
              <div className="flex justify-between items-start mb-8"><div className="w-14 h-14 bg-slate-900 rounded-[1.25rem] flex items-center justify-center text-blue-400 shadow-xl shadow-slate-900/20 group-hover:scale-110 transition-transform"><Icon name="calendar" size={26}/></div></div>
  ...
  return (
    <div className="max-w-6xl mx-auto animate-fade-in">
      {role === 'admin' && <button onClick={() => setSelectedCompanyId(null)} className="flex items-center gap-2 text-xs md:text-sm font-bold uppercase mb-8 text-slate-400 hover:text-slate-900 transition-colors tracking-widest"><Icon name="chevronLeft" size={16}/> Volver al Resumen</button>}
      ...
      <div className="glass-card p-8 md:p-10 bg-gradient-to-br from-blue-900 to-slate-900 text-white mb-8 relative overflow-hidden flex flex-col md:flex-row justify-between items-start md:items-center gap-6 shadow-2xl shadow-blue-900/20 border-none">
        ...
        <div className="z-10"><p className="text-xs font-black uppercase tracking-[0.2em] text-blue-400 mb-2">Panel de Control Inteligente</p><h2 className="text-4xl md:text-5xl font-black tracking-tighter leading-none text-white">{comp?.nombre}</h2></div>
      </div>
      ...
      {companyProjections && (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-10">
          <div className={`glass-card p-8 lg:col-span-1 flex flex-col h-full border-t-[8px] ${companyProjections.currentFuel <= companyProjections.criticalLiters ? 'border-t-red-500 bg-red-50/50 shadow-red-500/10' : 'border-t-yellow-400 shadow-sm'}`}>
            <div className={`w-16 h-16 rounded-[1.25rem] flex items-center justify-center shadow-inner mb-8 ${companyProjections.currentFuel <= companyProjections.criticalLiters ? 'bg-white text-red-500 border border-red-100 animate-pulse' : 'bg-slate-50 text-yellow-500 border border-slate-100'}`}><Icon name="fuel" size={32}/></div>
            ...
            <div className="space-y-5 mt-auto">
              <div className="flex justify-between items-end border-b border-slate-200/60 pb-3"><span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Fecha Límite</span><span className={`text-xl font-black ${companyProjections.currentFuel <= companyProjections.criticalLiters ? 'text-red-600' : 'text-slate-800'}`}>{companyProjections.fuelEstDate ? companyProjections.fuelEstDate.toLocaleDateString() : 'N/A'}</span></div>
              <div className="flex justify-between items-end border-b border-slate-200/60 pb-3"><span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Consumo Medio</span><span className="text-xl font-black mono text-slate-700">{companyProjections.fuelAvg > 0 ? companyProjections.fuelAvg.toFixed(1) : 0} L/DÍA</span></div>
              <div className="flex justify-between items-end bg-white p-4 rounded-xl border border-slate-100 shadow-sm mt-4"><span className="text-[10px] font-black uppercase tracking-widest text-slate-500">Stock Actual</span><span className="text-2xl font-black mono text-slate-900">{companyProjections.currentFuel.toFixed(0)} L</span></div>
            </div>
          </div>
