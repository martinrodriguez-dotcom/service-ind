    ...
    <div className="max-w-4xl mx-auto animate-fade-in space-y-6">
        ...
        <div className="grid grid-cols-1 gap-5">
            {alerts.map((a, i) => (
                <div 
                    key={i} 
                    className={`glass-card p-5 md:p-6 flex flex-col md:flex-row items-center gap-5 border-l-[8px] md:border-l-[12px] ${
                        a.type === 'BREAKDOWN' ? 'border-l-red-500' : 'border-l-yellow-400'
                    }`}
                >
                    <div className={`w-14 h-14 rounded-[1rem] flex items-center justify-center shrink-0 shadow-inner ${
                        a.type === 'BREAKDOWN' ? 'bg-red-50 text-red-500 animate-pulse' : 'bg-yellow-50 text-yellow-600'
                    }`}>
                    ...
                    <div className="flex-1 text-center md:text-left w-full">
                        ...
                        <p className="text-[10px] md:text-xs font-bold text-slate-500 uppercase tracking-widest truncate mt-1">
                            {a.companyName}
                        </p>
                    </div>
                                ...
                                <div className="flex flex-col sm:flex-row bg-slate-100/50 p-1.5 rounded-xl w-full gap-2 border border-slate-200/50">
                                    {['PENDIENTE', 'REVISION', 'REPARADO'].map(s => (
                                        <button 
                                            ...
                                            className={`px-4 py-2.5 rounded-[0.85rem] text-[10px] font-black uppercase w-full transition-all ${
                                                a.workflowStatus === s 
            ...
            {alerts.length === 0 && (
                <div className="text-center p-12 bg-white/40 backdrop-blur-md rounded-[2rem] border border-emerald-100 shadow-xl shadow-emerald-900/5">
                    ...
                    <p className="font-black text-xl text-emerald-700 tracking-tight">
                        Todo el sistema en orden
                    </p>
                    <p className="text-sm text-slate-500 font-medium mt-2">
                        No hay alertas críticas en la flota en este momento.
                    </p>
                </div>
            )}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {companies.map(emp => (
                <div key={emp.id} className="glass-card p-6 md:p-8 relative group flex flex-col h-full">
                    ...
                    <h3 className="text-2xl font-black tracking-tight text-slate-800 truncate pr-16 mb-2">
                        {emp.nombre}
                    </h3>
            ...
            <div className="max-w-6xl mx-auto animate-fade-in">
                ...
                <div className="glass-card p-8 md:p-10 bg-gradient-to-br from-slate-900 to-slate-800 text-white mb-8 relative overflow-hidden flex flex-col md:flex-row justify-between items-start md:items-center gap-6 shadow-2xl shadow-slate-900/20 border-none">
                    ...
                    <div className="flex flex-wrap items-center gap-4 z-10 w-full md:w-auto">
                        ...
                        >
                            <span className="text-[8px] font-black uppercase text-yellow-400 tracking-widest mb-1">
                                Ver Tanque
                            </span>
                    ...
                    {(activeCompany.vehiculos || []).map((v, index) => {
                        ...
                        return (
                            ...
                            >
                                ...
                                <h4 className="font-black text-xl tracking-tight text-slate-800 truncate mb-1">
                                    {v.nombre}
                                </h4>
                                <p className="font-bold text-slate-500 text-sm mono mb-6">
                                    {(v.horometroTotal || 0).toLocaleString()} 
                                    <span className="text-[10px] text-slate-400 uppercase tracking-widest ml-1">Horas</span>
                                </p>
                                ...
                                <div className="space-y-2 mt-auto bg-slate-50 p-4 rounded-2xl border border-slate-100/50">
                                    <div className="flex justify-between items-end">
                                        <span className="text-[9px] font-bold uppercase text-slate-400 tracking-widest">
                                            Vida Útil ({v.serviceInterval}h)
                                        </span>
                                        <span className={`text-[11px] font-black mono ${
                                            perc < 20 ? 'text-red-500' : 'text-slate-800'
                                        }`}>
                                            {perc.toFixed(0)}%
                                        </span>
                                    </div>
    ...
    return (
        <div className="max-w-4xl mx-auto animate-fade-in pb-10">
            ...
            <div className={`glass-card p-8 md:p-12 relative overflow-hidden shadow-2xl ${
                isTank ? 'border-t-[12px] border-t-emerald-500' : 'border-t-[12px] border-t-yellow-400'
            }`}>
                <div className={`absolute top-0 right-8 text-slate-900 px-4 py-2 rounded-b-xl font-black text-sm shadow-md tracking-widest ${
                    isTank ? 'bg-emerald-400' : 'bg-yellow-400'
                }`}>
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-8 mb-10 border-b border-slate-100 pb-10 mt-4">
                    <div className="flex items-center gap-6">
                        <div className="w-20 h-20 bg-gradient-to-br from-slate-900 to-slate-800 rounded-[1.5rem] flex items-center justify-center shadow-xl shadow-slate-900/20 text-yellow-400">
                            {isTank ? (
                                <Icon name="fuel" size={36} className="text-emerald-400"/>
                            ) : (
                                <span className="font-black text-3xl md:text-4xl">#{vehNumber}</span>
                            )}
                        </div>
                        <div>
                            <h4 className="font-black text-4xl md:text-5xl tracking-tighter text-slate-800">
                                {activeVehicle.nombre}
                            </h4>
                            <p className="font-bold text-slate-400 text-sm md:text-base uppercase tracking-widest mt-2">
                                {activeCompany.nombre}
                            </p>
                        </div>
                    </div>
                    ...
                    <div className="bg-slate-50 p-5 rounded-[1.5rem] text-center w-full md:w-auto border border-slate-200/60 shadow-inner">
                        <p className="text-[10px] font-black uppercase text-slate-400 tracking-widest mb-1">
                            {isTank ? 'Stock Actual' : 'Horómetro Actual'}
                        </p>
                        <p className="font-black text-slate-800 text-4xl mono">
                            {(activeVehicle.horometroTotal || 0).toLocaleString()} 
                            <span className="text-sm text-slate-400 uppercase tracking-widest ml-2">
                                {isTank ? 'L' : 'HS'}
                            </span>
                        </p>
                    </div>
                    ...
                    <div className={`grid gap-4 mb-10 ${isTank ? 'grid-cols-2 sm:grid-cols-3' : 'grid-cols-2 sm:grid-cols-4'}`}>
                        ...
                        {!isTank && (
                            <button 
                                ...
                                className="btn-accent px-4 py-5 rounded-2xl text-xs font-black tracking-widest w-full"
                            >
                                SERVICE
                            </button>
                        )}
                        ...
                        <button 
                            ...
                            className={`px-4 py-5 rounded-2xl font-black text-xs tracking-widest uppercase transition-all w-full shadow-md ${
                                activeVehicle.operativo 
            ...
            <div className="glass-card p-8 md:p-10 space-y-8">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                        ...
                        <input 
                            className="input-premium px-5 py-4 w-full font-black text-lg text-slate-800" 
                    ...
                    <div className="space-y-2">
                        ...
                        <input 
                            className="input-premium px-5 py-4 w-full text-sm font-medium text-slate-600" 
                ...
                <button 
                    ...
                    className="btn-premium px-8 py-4 rounded-xl text-sm font-black tracking-widest w-full md:w-auto"
                >
                    GUARDAR CAMBIOS
                </button>
            </div>
        ...
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4 border-b border-slate-200 pb-4">
                ...
                <button 
                    ...
                    className="btn-accent px-6 py-3.5 rounded-xl text-xs font-black tracking-widest w-full sm:w-auto"
                >
                    + NUEVO USUARIO
                </button>
            </div>
                    ...
                    <tbody>
                        {usersList.map(u => (
                            <tr key={u.uid} className="hover:bg-slate-50/50 transition-colors">
                                <td className="font-bold text-slate-800 text-sm truncate max-w-[150px]">
                                    {u.nombre || '-'}
                                </td>
                                <td className="font-medium text-slate-500 text-sm truncate max-w-[200px]">
                                    {u.email}
                                </td>
                                <td>
...
const CalendarView = ({ 
    ...
    if (!selectedCompanyId) {
        return (
            <div className="max-w-6xl mx-auto animate-fade-in">
                <h2 className="text-3xl md:text-4xl font-black tracking-tight text-slate-800 mb-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {(role === 'admin' ? companies : companies.filter(c => c.id === userCompanyId)).map(emp => (
                        ...
                        >
                            ...
                            <h3 className="text-2xl font-black tracking-tight text-slate-800 truncate mb-2 pr-12">
                                {emp.nombre}
                            </h3>
                            <p className="text-xs font-bold text-blue-500 uppercase tracking-widest flex items-center gap-2">
                                Analizar Datos <Icon name="chevronRight" size={14}/>
                            </p>
                        </div>
                    ))}
                </div>
        ...
        <div className="max-w-6xl mx-auto animate-fade-in">
            ...
            {companyProjections && (
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-10">
                    <div className={`glass-card p-8 lg:col-span-1 flex flex-col h-full border-t-[8px] ${
                        companyProjections.currentFuel <= companyProjections.criticalLiters 
                            ? 'border-t-red-500 bg-red-50/50 shadow-red-500/10' 
                            : 'border-t-yellow-400 shadow-sm'
                    }`}>
                        <div className={`w-16 h-16 rounded-[1.25rem] flex items-center justify-center shadow-inner mb-8 ${
                            companyProjections.currentFuel <= companyProjections.criticalLiters 
                                ? 'bg-white text-red-500 border border-red-100 animate-pulse' 
                                : 'bg-slate-50 text-yellow-500 border border-slate-100'
                        }`}>
                        ...
                        <div className="space-y-5 mt-auto">
                            ...
                            <div className="flex justify-between items-end border-b border-slate-200/60 pb-3">
                                ...
                                <span className="text-xl font-black mono text-slate-700">
                                    {companyProjections.fuelAvg > 0 ? companyProjections.fuelAvg.toFixed(1) : 0} L/DÍA
                                </span>
                            </div>
                    ...
                    <div className="glass-card p-8 lg:col-span-2 flex flex-col h-full shadow-sm">
                        ...
                        <div className="space-y-4 max-h-[400px] overflow-y-auto pr-3">
                            {companyProjections.vProjs.map((p, i) => {
                                ...
                                <div 
                                    key={i} 
                                    className={`p-6 rounded-[1.5rem] border-l-[8px] shadow-sm flex flex-col sm:flex-row justify-between gap-6 items-start sm:items-center transition-transform hover:-translate-y-0.5 ${
                                        p.restHs <= 50 
                                            ? 'border-l-red-500 bg-red-50/80 border border-red-100/50' 
                                            : 'border-l-blue-500 bg-slate-50/80 border border-slate-100/50'
                                    }`}
                                >
                                    <div className="flex items-center gap-5">
                                        <div className={`w-14 h-14 rounded-2xl flex items-center justify-center shrink-0 shadow-inner ${
                                            p.restHs <= 50 ? 'bg-white text-red-500' : 'bg-white text-blue-500'
                                        }`}>
                                            <span className="font-black text-xl">#{vehNumber}</span>
                                        </div>
                                        <div>
                                            <p className="font-black text-xl tracking-tight text-slate-800 truncate">
                                                {p.nombre}
                                            </p>
                                            <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-1">
                                                Uso: {p.avgUsage.toFixed(1)} HS/DÍA
                                            </p>
                                        </div>
                                    </div>
                                    <div className="bg-white px-5 py-4 rounded-xl border border-slate-200/60 text-center w-full sm:w-auto shadow-sm">
                                        ...
                                        <p className={`font-black mono text-2xl leading-none ${
                                            p.restHs <= 50 ? 'text-red-600' : 'text-slate-800'
                                        }`}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="glass-card p-8 flex flex-col h-full border-t-[8px] border-t-blue-500 shadow-lg shadow-blue-500/5">
                <div className="flex justify-between items-start mb-6">
                    ...
                    <button 
                        onClick={() => toggleInfo('lh')} 
                        className="w-8 h-8 rounded-full bg-slate-100 text-slate-500 flex items-center justify-center font-black text-sm hover:bg-blue-500 hover:text-white transition-colors shadow-sm"
                    >
                        i
                    </button>
                </div>
                {expandedInfo['lh'] && (
                    <p className="text-xs font-bold text-blue-800 bg-blue-50 p-4 rounded-xl mb-6 leading-relaxed border border-blue-100">
                        Mide litros consumidos por hora trabajada. Picos altos advierten fallas de inyección o posible robo de combustible (ordeñe).
                    </p>
                )}
                ...
                <div className="space-y-3 mt-auto">
                    {intelligenceData.topConsumidores.map((v, i) => (
                        <div key={i} className="flex justify-between items-center bg-slate-50 p-4 rounded-xl border border-slate-200/60 hover:border-blue-300 transition-colors">
                            <div>
                                <p className="font-black text-sm md:text-base text-slate-800">{v.nombre}</p>
                                <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">{v.companyName}</p>
                            </div>
                            <span className={`font-black mono text-lg md:text-xl bg-white px-3 py-1.5 rounded-lg border shadow-sm ${
                                v.lPh > 10 ? 'text-red-600 border-red-200' : 'text-slate-800 border-slate-200'
                            }`}>
                                {v.lPh} <span className="text-[10px] text-slate-400">L/H</span>
                            </span>
                        </div>
                    ))}
                </div>
            ...
            <div className="glass-card p-8 flex flex-col h-full border-t-[8px] border-t-yellow-400 shadow-lg shadow-yellow-400/5">
                <div className="flex justify-between items-start mb-6">
                    ...
                    <button 
                        onClick={() => toggleInfo('gastos')} 
                        className="w-8 h-8 rounded-full bg-slate-100 text-slate-500 flex items-center justify-center font-black text-sm hover:bg-yellow-400 hover:text-slate-900 transition-colors shadow-sm"
                    >
                        i
                    </button>
                </div>
            ...
            <div className="glass-card p-8 flex flex-col h-full border-t-[8px] border-t-emerald-500 shadow-lg shadow-emerald-500/5">
                <div className="flex justify-between items-start mb-6">
                    ...
                    <button 
                        onClick={() => toggleInfo('finanzas')} 
                        className="w-8 h-8 rounded-full bg-slate-100 text-slate-500 flex items-center justify-center font-black text-sm hover:bg-emerald-500 hover:text-white transition-colors shadow-sm"
                    >
                        i
                    </button>
                </div>
                {expandedInfo['finanzas'] && (
                    <p className="text-xs font-bold text-emerald-800 bg-emerald-50 p-4 rounded-xl mb-6 leading-relaxed border border-emerald-200">
                        Total de dinero invertido en el ciclo de vida de los equipos (basado en facturas de Services y Reparaciones).
                    </p>
                )}
            ...
            <div className="glass-card p-8 flex flex-col h-full border-t-[8px] border-t-slate-800 shadow-lg shadow-slate-500/5">
                <div className="flex justify-between items-start mb-6">
                    <h3 className="text-2xl font-black tracking-tight text-slate-800">Tasa de Uso (OEE)</h3>
                    <button 
                        onClick={() => toggleInfo('oee')} 
                        className="w-8 h-8 rounded-full bg-slate-100 text-slate-500 flex items-center justify-center font-black text-sm hover:bg-slate-800 hover:text-white transition-colors shadow-sm"
                    >
                        i
                    </button>
                </div>
                {expandedInfo['oee'] && (
                    <p className="text-xs font-bold text-slate-700 bg-slate-100 p-4 rounded-xl mb-6 leading-relaxed border border-slate-300">
                        Promedio de horas trabajadas por día. Equipos con bajo uso son capital inmovilizado y conviene reasignarlos.
                    </p>
                )}
                
                <div className="space-y-3 mt-auto">
                    {intelligenceData.topUso.map((v, i) => (
                        <div key={i} className="flex justify-between items-center bg-slate-50 p-4 rounded-xl border border-slate-200/60 hover:border-slate-300 transition-colors">
                            <div>
                                <p className="font-black text-sm md:text-base text-slate-800">{v.nombre}</p>
                                <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">{v.companyName}</p>
                            </div>
                            <span className="font-black mono text-lg md:text-xl bg-white px-3 py-1.5 rounded-lg border border-slate-200 shadow-sm text-slate-800">
                                {v.avgDiario} <span className="text-[10px] text-slate-400">HS/DÍA</span>
                            </span>
                        </div>
                    ))}
                </div>
