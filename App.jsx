...
const App = () => {
  ...
  const [newUser, setNewUser] = useState({ email: '', password: '', nombre: '', role: 'operario', companyId: '' });
          ...
          try {
            ...
            if(userDoc.exists()) {
              ...
              if (userData.role === 'suspendido') {
                await fb.signOut(auth); alert("Cuenta suspendida por administración."); return;
              }
              ...
              await fb.setDoc(userDocRef, { email: u.email, nombre: 'Admin Master', role: 'admin', requiresPasswordChange: false });
              setRole('admin'); setUserName('Admin Master');
            }
          } catch (e) { 
            setRole('operario'); setUserName('Operario BND');
          }
  ...
  useEffect(() => {
    if (role === 'operario' && userCompanyId && activeTab === 'companies' && !selectedCompanyId) setSelectedCompanyId(userCompanyId);
    if (role === 'operario' && userCompanyId && activeTab === 'calendar' && !selectedCompanyId) setSelectedCompanyId(userCompanyId);
  }, [role, userCompanyId, activeTab, selectedCompanyId]);

  const handleLogin = async (e) => {
    e.preventDefault(); setAuthLoading(true); setLoginError('');
  ...
  const handleLogout = async () => { 
    await fb.signOut(auth); setActiveTab('dashboard'); setActiveVehicleId(null); setSelectedCompanyId(null); setIsolatedVehicleId(null);
  };
  ...
  const handleRegisterUser = async (e) => {
    ...
    try {
      ...
      await fb.setDoc(fb.doc(db, "artifacts", APP_ID, "public", "data", "users", data.localId), {
        nombre: newUser.nombre, role: newUser.role, email: newUser.email, companyId: newUser.role === 'operario' ? newUser.companyId : null, requiresPasswordChange: true
      });
  ...
  // VEHÍCULO ACTIVO: Si el ID es TANK, creamos un vehículo virtual en memoria para reutilizar el modal.
  const activeVehicle = useMemo(() => {
    if (activeVehicleId === 'TANK' && activeCompany) {
        return {
            id: 'TANK', nombre: 'TANQUE PRINCIPAL', operativo: activeCompany.tanqueOperativo ?? true,
            horometroTotal: activeCompany.currentFuel, serviceInterval: activeCompany.tankCapacity,
            eventos: activeCompany.eventosTanque || []
        };
    }
  ...
  const displayedVehicles = useMemo(() => {
    ...
    if (isolatedVehicleId && isolatedVehicleId !== 'TANK') return activeCompany.vehiculos.filter(v => v.id === isolatedVehicleId);
  ...
  const alerts = useMemo(() => {
    ...
    relevantCompanies.forEach(emp => {
      ...
      if (emp.tanqueOperativo === false) list.push({ type: 'BREAKDOWN', id: 'TANK', nombre: 'TANQUE PRINCIPAL', companyId: emp.id, companyName: emp.nombre, workflowStatus: 'PENDIENTE', breakageMotive: (emp.eventosTanque || []).slice().reverse().find(e => e.tipo === 'BAJA')?.motivo || "Falla en tanque" });
  ...
  const intelligenceData = useMemo(() => {
    ...
    return { 
      ...
      eventCounts, breaksPerCompany, topUso: [...allVehicles].sort((a,b) => b.avgDiario - a.avgDiario).slice(0, 5) 
    };
  }, [companies]);

  const chartGastosData = { labels: ['Cargas Diésel', 'Services', 'Roturas/Reparaciones'], datasets: [{ data: [intelligenceData.eventCounts.REGISTRO, intelligenceData.eventCounts.SERVICE, intelligenceData.eventCounts.REPARACION], backgroundColor: ['#3b82f6', '#10b981', '#ef4444'], borderWidth: 0 }] };
  const chartSeveridadData = { labels: Object.keys(intelligenceData.breaksPerCompany), datasets: [{ label: 'Nº Roturas Históricas', data: Object.values(intelligenceData.breaksPerCompany), backgroundColor: '#f59e0b', borderRadius: 8 }] };
  ...
  const handleDeleteCompany = async (id) => { if(confirm("¿Eliminar la empresa y TODA su flota?")) await fb.deleteDoc(fb.doc(db, "artifacts", APP_ID, "public", "data", "companies", id)); };
  ...
  const handleAddCompany = async () => { 
    ...
    await fb.addDoc(fb.collection(db, "artifacts", APP_ID, "public", "data", "companies"), { 
      nombre: form.nombre, cuit: form.cuit, responsable: form.responsable, tankCapacity: parseFloat(form.tankCapacity) || 1000, criticalFuelPerc: parseFloat(form.criticalFuelPerc) || 25, currentFuel: parseFloat(form.tankCapacity) || 1000, 
      tanqueOperativo: true, eventosTanque: [], vehiculos: [] 
    }); 
  ...
  const handleAddVehicle = async () => { 
    ...
    await fb.updateDoc(fb.doc(db, "artifacts", APP_ID, "public", "data", "companies", activeCompany.id), { 
      vehiculos: fb.arrayUnion({ id: Date.now().toString(), numeroInterno: nuevoNumero, ...form, horometroTotal: h, ultimoServiceHoras: h, operativo: true, workflowStatus: 'PENDIENTE', eventos: [{ id: Date.now(), tipo: 'ALTA', fecha: new Date().toLocaleDateString(), horas: h, nota: 'Alta inicial del equipo' }] }) 
    }); 
    setModalType(null); 
  };
  ...
  const handleDailyLog = async () => {
    ...
    // CASO 1: Ingreso de Diésel al Tanque Central
    if (activeVehicleId === 'TANK') {
        const event = { id: Date.now(), tipo: 'INGRESO', fecha: form.fecha, litros: l, nota: form.nota || 'Provisión Externa' };
        await fb.updateDoc(fb.doc(db, "artifacts", APP_ID, "public", "data", "companies", activeCompany.id), { 
            currentFuel: (activeCompany.currentFuel || 0) + l,
            eventosTanque: fb.arrayUnion(event)
        });
    } 
    // CASO 2: Carga Diaria a una Máquina (Descuenta del tanque)
    else {
        ...
        const tankEvent = { id: Date.now(), tipo: 'DESCARGA', fecha: form.fecha, litros: l, nota: `Carga a equipo #${activeVehicle.numeroInterno || '-'} (${activeVehicle.nombre})` };
  ...
  const handleToggleStatus = async (vId, active) => { 
    if(active) { setActiveVehicleId(vId); setModalType('downtime'); } 
        ...
        if (vId === 'TANK') {
            await fb.updateDoc(fb.doc(db, "artifacts", APP_ID, "public", "data", "companies", activeCompany.id), { 
                tanqueOperativo: true, eventosTanque: fb.arrayUnion({ id: Date.now(), tipo: 'ALTA', fecha: new Date().toLocaleDateString(), nota: 'Tanque nuevamente operativo' }) 
            });
        } else {
            await fb.updateDoc(fb.doc(db, "artifacts", APP_ID, "public", "data", "companies", activeCompany.id), { vehiculos: activeCompany.vehiculos.map(v => v.id === vId ? { ...v, operativo: true, workflowStatus: 'PENDIENTE', eventos: [...(v.eventos || []), { id: Date.now(), tipo: 'ALTA', fecha: new Date().toLocaleDateString(), nota: 'Puesta en marcha manual' }] } : v) }); 
        }
    } 
  };
  ...
  const handleConfirmDowntime = async () => { 
    if (activeVehicleId === 'TANK') {
        await fb.updateDoc(fb.doc(db, "artifacts", APP_ID, "public", "data", "companies", activeCompany.id), { 
            tanqueOperativo: false, eventosTanque: fb.arrayUnion({ id: Date.now(), tipo: 'BAJA', fecha: new Date().toLocaleDateString(), motivo: form.motivo }) 
        });
    } else {
        await fb.updateDoc(fb.doc(db, "artifacts", APP_ID, "public", "data", "companies", activeCompany.id), { vehiculos: activeCompany.vehiculos.map(v => v.id === activeVehicleId ? { ...v, operativo: false, workflowStatus: 'PENDIENTE', eventos: [...(v.eventos || []), { id: Date.now(), tipo: 'BAJA', fecha: new Date().toLocaleDateString(), motivo: form.motivo }] } : v) }); 
    }
  ...
  const handleUpdateWorkflow = async (cId, vId, ns) => { 
    if (vId === 'TANK') {
        ...
        if(ns === 'REPARADO') { setActiveVehicleId(vId); setSelectedCompanyId(cId); setModalType('repair_finish'); return; } 
        await fb.updateDoc(fb.doc(db, "artifacts", APP_ID, "public", "data", "companies", cId), { vehiculos: companies.find(x => x.id === cId).vehiculos.map(v => v.id === vId ? {...v, workflowStatus: ns} : v) }); 
    }
  };
  ...
  const handleRepairSubmit = async () => { 
    if (activeVehicleId === 'TANK') {
        await fb.updateDoc(fb.doc(db, "artifacts", APP_ID, "public", "data", "companies", selectedCompanyId), { 
            tanqueOperativo: true, eventosTanque: fb.arrayUnion({ id: Date.now(), tipo: 'REPARACION', fecha: new Date().toLocaleDateString(), nota: form.nota }) 
        });
    } else {
        await fb.updateDoc(fb.doc(db, "artifacts", APP_ID, "public", "data", "companies", selectedCompanyId), { vehiculos: companies.find(c => c.id === selectedCompanyId).vehiculos.map(v => v.id === activeVehicleId ? { ...v, operativo: true, workflowStatus: 'PENDIENTE', eventos: [...(v.eventos || []), { id: Date.now(), tipo: 'REPARACION', fecha: new Date().toLocaleDateString(), nota: form.nota }] } : v) }); 
    }
    setModalType(null); 
  };
  ...
  const handleHistoricalData = async () => { 
    ...
    await fb.updateDoc(fb.doc(db, "artifacts", APP_ID, "public", "data", "companies", activeCompany.id), { vehiculos: activeCompany.vehiculos.map(v => v.id === activeVehicleId ? { ...v, horometroTotal: Math.max(v.horometroTotal, h), eventos: [...(v.eventos || []), { id: Date.now(), tipo: 'REGISTRO', fecha: form.fecha, horas: h, nota: "Sincronización Histórica" }] } : v) }); setModalType(null); 
  };

  const handleServiceReset = async () => { 
    await fb.updateDoc(fb.doc(db, "artifacts", APP_ID, "public", "data", "companies", activeCompany.id), { vehiculos: activeCompany.vehiculos.map(x => x.id === activeVehicleId ? { ...x, ultimoServiceHoras: x.horometroTotal, eventos: [...(x.eventos || []), { id: Date.now(), tipo: 'SERVICE', fecha: new Date().toLocaleDateString(), horas: x.horometroTotal, insumos: form.insumos, nota: form.nota }] } : x) }); setModalType(null); setForm({...form, insumos: [], nota: ''}); 
  };
  ...
  const downloadPDF = (company, vehicle) => {
    ...
    doc.setTextColor(0, 0, 0); doc.text(`Cliente: ${company.nombre}`, 15, 55); doc.text(`Equipo: ${vehicle.nombre}`, 15, 62);
    const body = (vehicle.eventos || []).slice().reverse().map(e => [e.fecha, e.tipo, e.horas ? `${e.horas} HS` : '-', e.litros || '-', e.nota || e.motivo || '-']);
    doc.autoTable({ startY: 70, head: [['FECHA', 'TIPO', 'LECTURA', 'LITROS', 'DETALLE']], body, headStyles: { fillColor: [15, 23, 42] } });
      ...
      scannerRef.current.start(
        ...
        (txt) => {
          if (txt.startsWith("sp-asset:")) { 
            ...
            if (role === 'operario' && cid !== userCompanyId) { alert("Acceso denegado: Este equipo no pertenece a tu flota."); setScannerActive(false); scannerRef.current.stop(); return; }
  ...
  return (
    <div className="flex h-[100dvh] bg-slate-50 overflow-hidden select-none w-full" style={{ paddingTop: 'env(safe-area-inset-top)', paddingBottom: 'env(safe-area-inset-bottom)', paddingLeft: 'env(safe-area-inset-left)', paddingRight: 'env(safe-area-inset-right)' }}>
      ...
      <window.AppModals 
        ...
        handlePrintQR={handlePrintQR} chartRef={chartRef}
      />
