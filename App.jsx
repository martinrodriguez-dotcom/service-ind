...
const App = () => {
    // 1. IMPORTACIONES SEGURAS DESDE WINDOW
    const { 
        Icon, 
        FuelTankCapsule, 
        db, 
    ...
    const handleChangePassword = async (e) => {
        ...
        try {
            ...
            setPwdError('Error al actualizar.'); 
        }
    };
    ...
    const activeVehicle = useMemo(() => {
        if (activeVehicleId === 'TANK' && activeCompany) {
            return {
                ...
                operativo: activeCompany.tanqueOperativo ?? true,
                horometroTotal: activeCompany.currentFuel, 
                serviceInterval: activeCompany.tankCapacity,
    ...
    const displayedVehicles = useMemo(() => {
        ...
        if (isolatedVehicleId && isolatedVehicleId !== 'TANK') {
        ...
        relevantCompanies.forEach(emp => {
            ...
            if (emp.tanqueOperativo === false) {
                list.push({ 
                    ...
                    breakageMotive: (emp.eventosTanque || []).slice().reverse().find(e => e.tipo === 'BAJA')?.motivo || "Falla en tanque" 
                });
            }
    ...
    const intelligenceData = useMemo(() => {
        ...
        relevantCompanies.forEach(comp => {
            ...
            (comp.vehiculos || []).forEach(v => {
                ...
                if (regs.length >= 2) {
                    const dD = Math.max(1, (new Date(regs[regs.length-1].fecha) - new Date(regs[0].fecha)) / 86400000);
                    avgDiario = hsTrabajadas / dD;
                }

                allVehicles.push({ 
                    id: v.id, 
                    nombre: v.nombre, 
                    companyName: comp.nombre, 
                    lPh: parseFloat(lPh.toFixed(2)), 
                    avgDiario: parseFloat(avgDiario.toFixed(1)) 
                });
    ...
    const handleDeleteCompany = async (id) => { 
        if(confirm("¿Eliminar la empresa y TODA su flota permanentemente?")) {
    ...
    const downloadPDF = (company, vehicle) => {
        ...
        doc.text(`Equipo: ${vehicle.nombre}`, 15, 62);
            ...
            <div className="flex h-[100dvh] items-center justify-center bg-slate-900 px-4 md:px-6 select-none w-full" style={{ paddingTop: 'env(safe-area-inset-top)', paddingBottom: 'env(safe-area-inset-bottom)' }}>
                <div className="glass-card p-8 md:p-12 w-full max-w-sm md:max-w-md bg-white/5 backdrop-blur-2xl border-white/10 text-center">
                    ...
                    <form onSubmit={handleLogin} className="space-y-4">
                        ...
                        />
                        <button type="submit" className="btn-accent w-full px-5 py-3 rounded-xl text-sm font-black mt-4 uppercase shadow-lg">
                            Entrar
                        </button>
                    </form>
                </div>
        ...
        <div className="flex h-[100dvh] bg-slate-50 overflow-hidden select-none w-full" style={{ paddingTop: 'env(safe-area-inset-top)', paddingBottom: 'env(safe-area-inset-bottom)', paddingLeft: 'env(safe-area-inset-left)', paddingRight: 'env(safe-area-inset-right)' }}>
            ...
            <aside className={`fixed inset-y-0 left-0 z-[200] w-72 bg-white flex flex-col p-6 md:p-8 border-r border-slate-100 transform transition-transform duration-300 lg:relative lg:translate-x-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
                ...
                <div className="pt-4 border-t border-slate-100 space-y-3 mt-auto">
                    <button onClick={startScanner} className="btn-accent px-4 py-3 w-full rounded-xl text-xs md:text-sm font-black shadow-md flex items-center justify-center gap-2">
                        <Icon name="qr" size={16}/> ESCANEAR
                    </button>
                    <button onClick={handleLogout} className="w-full px-4 py-3 text-[10px] font-black uppercase text-red-500 bg-red-50 rounded-xl hover:bg-red-100 transition-colors">
                        Cerrar Sesión
                    </button>
                </div>
            </aside>
            ...
            <main className="flex-1 flex flex-col h-full overflow-hidden">
                <header className="lg:hidden h-16 sm:h-20 flex items-center justify-between px-5 bg-white/90 backdrop-blur-md border-b border-slate-100 shrink-0 z-50">
                    <button onClick={() => setSidebarOpen(true)} className="p-2 bg-slate-50 rounded-lg active:scale-95">
                ...
                <div className="flex-1 overflow-y-auto p-4 md:p-8 lg:p-10 space-y-6 md:space-y-8 pb-32">
                    ...
                    {activeTab === 'companies' && activeCompany && CompanyDetailView && (
                        <CompanyDetailView 
                            role={role} 
                            activeCompany={activeCompany} 
                            activeVehicle={activeVehicle} 
                            activeVehicleId={activeVehicleId} 
                            isolatedVehicleId={isolatedVehicleId} 
                    ...
                    {activeTab === 'config' && role === 'admin' && ConfigView && (
                        <ConfigView 
                            ...
                            setBndSettings={setBndSettings} 
                            saveBndSettings={saveBndSettings} 
                            usersList={usersList} 
                            companies={companies} 
            ...
            {AppModals && (
                <AppModals 
                    ...
                    handleEditVehicleSubmit={handleEditVehicleSubmit} 
                    handleDailyLog={handleDailyLog} 
                    handleServiceReset={handleServiceReset} 
                    handleConfirmDowntime={handleConfirmDowntime} 
